...
- **New `/book` page (`public/book.html`).** Embeds SQUIRE's booking flow **directly in the page** as a full-width in-flow iframe of `getsquire.com/booking/book/al-hakeems-tonsorial-owings-mills-1/barber/clifton-al-hakeem-bey-1/services?platform=widget&viewMode=singleBarber` — the same URL SQUIRE's own widget frames. SQUIRE's `widget.js` loader is deliberately **not** used: reading its `frameLoader.js` source showed it offers only a floating launcher bubble plus a fixed slide-in dialog (`position: fixed`, parked off-screen via `translateX`), never an in-page embed, and its `x-squire-inline-enabled` flag does not change that. The embed keeps the third-party surface off every other page. Includes a `<noscript>` fallback and a visible "Book directly on SQUIRE" link to the hosted short link `https://getsqr.co/clifton-22`, plus the phone number. Note: the URL slug comes from SQUIRE — if the shop/barber route ever changes there, the embed dies gracefully (see fallback below) but the src should be updated.
