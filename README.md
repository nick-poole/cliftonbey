...
- **The booking page cannot silently fail.** A hidden fallback CTA on `/book` is revealed by a small inline script after 6s if SQUIRE's iframe has not appeared, and a `MutationObserver` retracts it again if the widget arrives late on a slow connection, so the "didn't load" message can never sit above a working widget. Detection matches SQUIRE's own markers (`iframe[src*="getsquire"]` / `iframe.squire_widget`), never just any iframe — Netlify's deploy-preview toolbar is an iframe and would otherwise mask a dead widget during preview testing. The script stays inline and independent of `main.js` so it runs regardless of the shared bundle's state.
- **Widget mount behavior (verified on deploy preview).** With `x-squire-inline-enabled`, the widget ignores local containers entirely: it appends its own `#squire_booking_widget_root` plus an `iframe.squire_widget` (src on the `getsquire.com` apex) to the end of `<body>`. The defensive placeholder container from the first pass was removed once this was confirmed.
...
- **CSP widened for SQUIRE.** `script-src`/`connect-src`/`frame-src`/`style-src`/`font-src` now allow `https://*.getsquire.com`, and `frame-src`/`connect-src` additionally allow the **apex** `https://getsquire.com`. Deploy-preview testing showed the booking iframe loads from the apex, which a `*.getsquire.com` wildcard does **not** match (CSP wildcards cover subdomains only) — the apex entry is what makes the widget render, so it must not be removed when tightening. Netlify applies every matching `[[headers]]` rule and browsers intersect duplicate CSP headers, so a `/book`-scoped policy would not have worked — the site-wide policy is the one that had to change.
