...
- **The booking page cannot silently fail.** A hidden fallback CTA sits below the widget mount; a small inline script on `/book` reveals it after 6s if neither the mount nor any injected SQUIRE element is present. A `MutationObserver` retracts it again if the widget arrives late on a slow connection, so the "didn't load" message can never sit above a working widget. The script stays inline and independent of `main.js` so it runs regardless of the shared bundle's state. If that CTA appears in production, the widget did not render — whether because the brand is not live yet, the request was blocked, or inline mode wanted a container we did not provide.
