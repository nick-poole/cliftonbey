...
- **Dedicated hero + social image for `/book`.** `assets/images/book-hero.webp` (1800x514 panoramic, black-and-white) is the page hero and LCP preload; `book-hero.jpg` is the `og:image`/`twitter:image` (1800x514 with matching width/height/alt tags). The current files are a grayscale derivative of `tonsorial-hero` standing in for the client-chosen photo — replace both files (same names) to swap the artwork with zero code changes.
