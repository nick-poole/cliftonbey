...
- **The booking page cannot silently fail.** A hidden fallback CTA sits below the widget mount; a small inline script on `/book` reveals it after 4s if neither the mount nor any injected SQUIRE element is present. It stays inline and independent of `main.js` so it runs regardless of the shared bundle's state. If that CTA ever appears in production, the widget did not render.
